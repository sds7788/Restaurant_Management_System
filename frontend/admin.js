...
/**
 * [已添加] 格式化日期时间字符串
 * @param {string | null | undefined} dateString - 从API获取的日期字符串
 * @returns {string} 格式化为 'YYYY-MM-DD HH:mm:ss' 的字符串，如果输入无效则返回 'N/A'
 */
 ...
 * @param {number} perPage - 每页项目数
 * @param {function} fetchFunction - 点击页码时调用的数据获取函数 (例如 fetchUsers)
 */
function renderPagination(containerId, totalItems, currentPage, perPage, fetchFunction) {
    ...
    const createButton = (text, page, enabled = true, isCurrent = false) => {
        ...
        button.className = `button text-sm py-1.5 px-3 mx-1 ${isCurrent ? 'bg-purple-700 text-white cursor-default' : 'button-secondary'}`;
        if (enabled && !isCurrent) {
            button.onclick = () => fetchFunction(page);
        } else {
    ...
    // 上一页按钮
    if (currentPage > 1) {
        container.appendChild(createButton('<i class="fas fa-chevron-left mr-1"></i> 上一页', currentPage - 1));
    }
    ...
    if (startPage > 1) {
        ...
        if (startPage > 2) {
            ...
            dots.textContent = '...';
            dots.className = 'px-3 py-1.5';
            container.appendChild(dots);
        }
    }

    for (let i = startPage; i <= endPage; i++) {
        container.appendChild(createButton(i, i, true, i === currentPage));
    }
    ...
    if (currentPage < totalPages) {
        container.appendChild(createButton('下一页 <i class="fas fa-chevron-right ml-1"></i>', currentPage + 1));
    }
}
    ...
    menuItems.forEach(item => {
        ...
        tableHtml += `
            <tr>
                ...
                <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">¥${Number(item.price).toFixed(2)}</td>
...
function renderOrderManagementTable(orders) {
    ...
    orders.forEach(order => {
        ...
        const paymentStatus = order?.payment_status || 'unpaid';  // 新增支付状态获取
        tableHtml += `
            <tr>
...
async function showOrderDetailsModal(orderId) {
    ...
    try {
        ...
        detailsHtml += `<p><strong>总金额:</strong> ¥${formatPrice(orderDetails?.total_amount)}</p>`;
...
// == 用户管理 ==
function loadUserManagement(page = 1) {
    mainContentTitle.textContent = '用户管理';
    setActiveNavLink(navUserManagement);
    adminContentArea.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow">
            <h2 class="text-xl font-semibold text-slate-700 mb-6">所有用户</h2>
            <div id="userManagementTableContainer" class="overflow-x-auto"></div>
            <div id="userPaginationContainer" class="mt-6 flex justify-center items-center"></div>
        </div>
    `;
    fetchUsers(page);
}
...
async function fetchUsers(page = 1) {
    ...
    container.innerHTML = `<p class="text-slate-500">正在加载用户列表...</p>`;

    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/admin/users?page=${page}&per_page=10`);
        if (!response.ok) throw new Error('获取用户列表失败');
        const data = await response.json();
        allUsersCache = data.users;
        renderUserTable(data.users);
        renderPagination('userPaginationContainer', data.total_users, data.page, data.per_page, fetchUsers);
    } catch (error) {
...
function renderUserTable(users) {
    ...
    if (!users || users.length === 0) {
        container.innerHTML = '<p class="text-slate-500">暂无用户。</p>';
        return;
    }
    container.innerHTML = `
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">全名</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Email</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">角色</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">注册时间</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">操作</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-slate-200">
            ${users.map(user => `
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${user.full_name || 'N/A'}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${user.email || 'N/A'}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm">
                        <span class="font-semibold ${user.role === 'admin' ? 'text-purple-600' : 'text-slate-600'}">${user.role}</span>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${formatDate(user.created_at)}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button class="text-purple-600 hover:text-purple-900" onclick="showUpdateUserRoleModal(${user.id}, '${user.role}')" ${user.id === currentAdmin.id ? 'disabled title="不能修改自己的角色"' : ''}><i class="fas fa-user-shield"></i> 改角色</button>
                        <button class="text-red-600 hover:text-red-900" onclick="confirmDeleteUser(${user.id})" ${user.id === currentAdmin.id ? 'disabled title="不能删除自己"' : ''}><i class="fas fa-trash-alt"></i> 删除</button>
                    </td>
                </tr>
            `).join('')}
            </tbody>
...
function showUpdateUserRoleModal(userId, currentRole) {
    const form = document.createElement('form');
    form.id = 'updateUserRoleForm';
    form.innerHTML = `
        <p class="mb-2">为用户ID #${userId} 选择新角色:</p>
        <select name="role" class="input-field w-full">
            <option value="customer" ${currentRole === 'customer' ? 'selected' : ''}>Customer</option>
            <option value="admin" ${currentRole === 'admin' ? 'selected' : ''}>Admin</option>
        </select>
    `;
    showAdminModal('修改用户角色', form, [
        { text: '取消', class: 'button-secondary', action: closeAdminModal },
        { text: '保存', class: 'button-primary', action: () => handleUpdateUserRole(userId) }
    ]);
}
...
async function handleUpdateUserRole(userId) {
    ...
    try {
        ...
        closeAdminModal();
        showAdminModal('成功', '用户角色已更新。');
        fetchUsers(currentPage);
    } catch(error) {
        showAdminModal('错误', error.message);
    }
}

function confirmDeleteUser(userId) {
    showAdminModal('确认删除用户', `确定要删除用户ID #${userId} 吗？此操作不可撤销。`, [
        { text: '取消', class: 'button-secondary', action: closeAdminModal },
        { text: '确认删除', class: 'button-danger', action: () => handleDeleteUser(userId) }
    ]);
}

async function handleDeleteUser(userId) {
    try {
        const response = await fetchWithAuth(`${API_BASE_URL}/admin/users/${userId}`, { method: 'DELETE' });
        const result = await response.json();
        if (!response.ok) throw new Error(result.error || '删除失败');
        
        closeAdminModal();
        showAdminModal('成功', '用户已删除。');
...
function loadCategoryManagement() {
    ...
    adminContentArea.innerHTML = `
        <div class="bg-white p-6 rounded-lg shadow">
            <div class="flex justify-between items-center mb-6">
                <h2 class="text-xl font-semibold text-slate-700">菜品分类</h2>
                <button id="addCategoryBtn" class="button button-primary py-2 px-4 flex items-center">
                    <i class="fas fa-plus mr-2"></i>添加新分类
                </button>
            </div>
            <div id="categoryManagementContainer" class="overflow-x-auto"></div>
        </div>
    `;
    document.getElementById('addCategoryBtn').addEventListener('click', () => showAddEditCategoryModal());
    fetchCategoriesForManagement();
}

async function fetchCategoriesForManagement() {
    const container = document.getElementById('categoryManagementContainer');
    container.innerHTML = `<p class="text-slate-500">正在加载分类列表...</p>`;
    try {
        await fetchAllCategories(); // 使用已有的函数来填充 allCategoriesCache
        renderCategoryTable(allCategoriesCache);
    } catch(error) {
        container.innerHTML = `<p class="text-red-500">${error.message}</p>`;
    }
}
...
function renderCategoryTable(categories) {
    ...
    if (!categories || categories.length === 0) {
        container.innerHTML = '<p class="text-slate-500">暂无分类，请添加一个新分类。</p>';
        return;
    }
    container.innerHTML = `
        <table class="min-w-full divide-y divide-slate-200">
            <thead class="bg-slate-50">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">显示顺序</th>
                    <th class="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">操作</th>
                </tr>
            </thead>
            <tbody class="bg-white divide-y divide-slate-200">
            ${categories.map(cat => `
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${cat.description || 'N/A'}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500">${cat.display_order}</td>
                    <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        <button class="text-purple-600 hover:text-purple-900" onclick="showAddEditCategoryModal(${cat.id})"><i class="fas fa-edit"></i> 编辑</button>
                        <button class="text-red-600 hover:text-red-900" onclick="confirmDeleteCategory(${cat.id})"><i class="fas fa-trash-alt"></i> 删除</button>
                    </td>
                </tr>
            `).join('')}
            </tbody>
...
function showAddEditCategoryModal(categoryId = null) {
    currentEditingCategoryId = categoryId;
    const isEditMode = categoryId !== null;
    const category = isEditMode ? allCategoriesCache.find(c => c.id === categoryId) : {};
    
    if (isEditMode && !category) {
        showAdminModal('错误', '找不到要编辑的分类。');
        return;
    }
    ...
    form.innerHTML = `
        <input type="text" name="name" placeholder="分类名称" value="${category?.name || ''}" required class="input-field w-full">
        <textarea name="description" placeholder="分类描述 (可选)" class="input-field w-full">${category?.description || ''}</textarea>
        <input type="number" name="display_order" placeholder="显示顺序" value="${category?.display_order ?? 0}" required class="input-field w-full">
    `;
    
    showAdminModal(isEditMode ? '编辑分类' : '添加新分类', form, [
        { text: '取消', class: 'button-secondary', action: closeAdminModal },
        { text: '保存', class: 'button-primary', action: () => handleSaveCategory() }
    ]);
}
...
async function handleSaveCategory() {
    ...
    const data = {
        ...
        display_order: parseInt(form.elements.display_order.value)
    };

    if (!data.name || isNaN(data.display_order)) {
        alert('请输入有效的分类名称和显示顺序。');
        return;
    }
    
    const url = currentEditingCategoryId ? `${API_BASE_URL}/admin/categories/${currentEditingCategoryId}` : `${API_BASE_URL}/admin/categories`;
    ...
    try {
        ...
        closeAdminModal();
        showAdminModal('成功', `分类已${currentEditingCategoryId ? '更新' : '添加'}！`);
        fetchCategoriesForManagement();
    } catch(error) {
        showAdminModal('错误', error.message);
    }
...
function confirmDeleteCategory(categoryId) {
    ...
    showAdminModal('确认删除分类', `确定要删除分类 "${category?.name}" 吗？如果该分类下仍有菜品，将无法删除。`, [
        { text: '取消', class: 'button-secondary', action: closeAdminModal },
        { text: '确认删除', class: 'button-danger', action: () => handleDeleteCategory(categoryId) }
    ]);
}
